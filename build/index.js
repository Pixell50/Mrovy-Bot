                ...
                if (player.voiceId === msg.member.voice.channel?.id) {
                    if (player.queue[0].requestedBy === msg.member || msg.member.hasPermission("ADMINISTRATOR")) {
                        if (player.status === PlayerStatus.PAUSED)
                            return msg.reply('Zaten durdurulmuş!');
                        msg.reply('Durdurdum!');
                        player.dispatcher?.pause();
                        player.status = PlayerStatus.PAUSED;
                    }
                    else
                        return msg.reply('Şu an çalan şarkıyı sadece `' + player.queue[0].requestedBy.user.tag + '` ya da yetkili birisi durdurabilir.');
                }
                else
                    return msg.reply('Şarkı çaldığım kanalda değilsin!');
            }
            else
                return msg.reply('Henüz şarkı çalmıyorum..');
            break;
        case 'devam':
            if (player) {
                if (player.voiceId === msg.member.voice.channel?.id) {
                    if (player.queue[0].requestedBy === msg.member || msg.member.hasPermission("ADMINISTRATOR")) {
                        if (player.status === PlayerStatus.PLAYING)
                            return msg.reply('Zaten devam ediyor!');
                        msg.reply('Devam ediyor!');
                        player.dispatcher?.resume();
                        player.status = PlayerStatus.PLAYING;
                    }
                    else
                        return msg.reply('Şu an çalan şarkıyı sadece `' + player.queue[0].requestedBy.user.tag + '` ya da yetkili birisi başlatabilir.');
                }
                else
                    return msg.reply('Şarkı çaldığım kanalda değilsin!');
            }
            ...
            if (player) {
                if (player.voiceId === msg.member.voice.channel?.id) {
                    if (player.queue[0].requestedBy === msg.member || msg.member.hasPermission("ADMINISTRATOR")) {
                        ...
                        return msg.reply('Şu an çalan şarkıyı sadece `' + player.queue[0].requestedBy.user.tag + '` ya da yetkili birisi geçebilir.');
                }
                else
                    return msg.reply('Şarkı çaldığım kanalda değilsin!');
            }
...
async function addQueue(guild, requestedBy, url) {
    ...
    const title = (await (0, ytdl_core_1.getInfo)(url)).videoDetails.title;
    guildPlayer.queue.push({ title, url, requestedBy });
    return true;
}
async function play(player, msg) {
